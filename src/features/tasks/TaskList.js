...
export default function TaskList({ tasks, statuses, connectedAddress, updateStatus, deleteTask, updateTask }) {
    ...
    <>
      ...
      {statuses.map((status) => (
        <div key={status} style={{ marginBottom: "30px" }}>
          <h2 style={{ borderBottom: "2px solid #eee", paddingBottom: "5px", fontWeight: "600", fontSize: "18px" }}>
            {status}
          </h2>
              ...
              .map((task) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => isAuthorized && setSelectedTask(task)} // ouvre la modal
                  >
                    <div style={{ flex: "1 1 200px", marginRight: "10px" }}>
                      ...
                      <small style={{ color: "#888" }}>
                        Collaborateurs: {collaborators.length ? collaborators.map(truncateAddress).join(", ") : "Aucun"}
                      </small>
                    </div>
                    ...
                    <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginTop: "10px" }}>
                      {isAuthorized ? (
                        ...
                        <span style={{ color: "#aaa", fontStyle: "italic" }}>Lecture seule</span>
                      )}
                    </div>
                  </div>
                );
