    ...
    <>
      {statuses.map(status => (
        <div key={status} style={{ marginBottom: "30px" }}>
          <h2 style={{ borderBottom: "2px solid #eee", paddingBottom: "5px", fontWeight: "600" }}>{status}</h2>
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {tasks.filter(t => t.status === statuses.indexOf(status)).map(task => (
              ...
              >
                <div>
                  <p style={{ margin: 0, fontWeight: "600", fontSize: "16px" }}>{task.title}</p>
                  <p style={{ margin: "4px 0", fontSize: "14px", color: "#555" }}>{task.description}</p>
                  <small style={{ color: "#888" }}>Créé par: {task.creator}</small>
                </div>
                <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                  {statuses.map((s, i) => i !== task.status && (
                    <button
                      key={i}
